...
A built SPA is **a workflow**: no server code. Register the assets as a
...
(`@pattern-js/mod-admin` is the living example of this same trio. It registers
imperatively because it ships as a package; an app author declares the
...
creates the first admin. Magic-link and invite links are
