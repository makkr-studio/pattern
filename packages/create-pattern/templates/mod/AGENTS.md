...
Routes are workflows that front a pure op. Use `httpEndpoint(...)` and name the
route; there is no generic "run any op" endpoint.
...
- **Tier 2**: a fully-custom React page shipped as `module` **source** (the ESM
  ...
  (the client), `ui` (the glass kit), `motion` (motion.dev) and `lucide`, giving
  it the admin's *exact* stack with no bundler. The admin serves that source
