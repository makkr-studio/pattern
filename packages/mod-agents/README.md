...
```bash
npm install @pattern/mod-agents
```
...
provider (`@pattern/mod-agents-openai`) that reifies the descriptors and emits
the turn events.
...
Always ship it with a provider. Usually also `@pattern/mod-store` (history +
blobs) and `@pattern/mod-vault` (the API key).
