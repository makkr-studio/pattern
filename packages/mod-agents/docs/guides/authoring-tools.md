...
A tool is a workflow with a `boundary.tool` trigger and a
`boundary.tool.return` out-gate. Reach for one whenever the model needs to
*do* something: look data up, call an API, or change state. Three things make
it a good tool.
...
shape becomes a clean `TriggerInputError` before any malformed value reaches
...
for every field. A weather tool returns `{ tempC, summary }`, the few fields
the model needs. `boundary.tool.return` is the only legal terminal for a tool
...
- **MCP servers**: `agents.mcp.client` exposes an existing MCP server's tools
...
`result` is `{ tripwire: boolean, info? }`. The run loop calls it as an input
...
1. Author the classifier as a `boundary.tool` workflow. Mark it
   `guardrail: true` so `agents.tools.workflows` keeps it **out** of the
   model's callable toolbox: the classifier vets messages without becoming a
