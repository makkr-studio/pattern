...
**Reach for a fork when the change is structural:** a different toolset per
assistant, an extra node (compaction, RAG retrieval, an output guardrail), or a
rewired reliability path. Anything you can express as one of the options above
stays no-fork.
...
nodes too, you can re-route reliability behavior alongside the agent. There's a
...
`approval.request`, `audio.ref`, a terminal `done`) through a store abstraction.
It never touches the wire. The voice/avatar surface plugs into that same store;
build any other surface the same way.
