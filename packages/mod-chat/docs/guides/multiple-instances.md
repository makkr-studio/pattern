...
load. (A single instance, with no `instances` array, keeps the canonical ids and
the `/chat` mount, unchanged.)
...
generic pipeline. No registry, no dispatch config. Pipeline selection is a
