...
import { memoryOps, memoryPipelineWorkflow, rememberToolWorkflow } from "./memory.js";
  ...
  const workflows = [
    ...
    ...(opts.turnPipeline
      ? [
          ...
          // turn, as its own (inspectable) run, and the agent gets a visible
          // `remember` tool. Both no-op without mod-vectors.
          ...(opts.memory.enabled ? [memoryPipelineWorkflow(), rememberToolWorkflow()] : []),
        ]
      : []),
