...
the internals. For the live op list see the [op reference](/docs/ops); for the
exact mechanics, the cited source files are the source of truth.

## Four defining properties

1. **Workflows are data, not code.** A workflow is a JSON document describing a
   ...
   versionable, and editable in the admin — and it's the basis of the mod system.
...
freely with value ports. The engine is not "all streams."

## The three edge kinds

Edge kind is **derived from the ports it connects**, never declared — and it's the
...
declare *named* control-outs and pulse them selectively instead. A node with wired
control-ins waits for **all** of them (AND semantics). A *declared* data port named
`in`/`out` shadows the implicit control port — which is why stream ops legitimately
...
- **Stream hub** — a backpressured broadcaster that tees one output to N consumers
  (high-water mark 1 per consumer, so a slow branch slows the source rather than
...
unreached regions settle instead of hanging. A value input with several producers
...
instead. On any node error or external cancellation the run aborts fast: the signal
...
Auth is **run context, not a boundary** — every entry point wants to know "who is
this running as." `ctx.principal` is first-class, defaulting to anonymous. Auth
providers form a registry **chain**; a trigger may declare `requireAuth` (a boolean
or `{ scopes }`), enforced by the host *before* the graph runs — required-but-
anonymous returns the boundary's unauthorized form (HTTP 401, CLI nonzero exit)
...
## Distribution — an invariant, not a feature
...
5. **Auth is run context, not a boundary** — `Principal` + provider chain + optional
   per-trigger requirement; user storage is a mod concern.
