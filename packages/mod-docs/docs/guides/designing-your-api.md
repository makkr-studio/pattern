...
You can build a working backend on Pattern several ways. Most of them are
...
- **Auth**: `requireAuth: { scopes: ["edit"] }` on the trigger gives you
  **401 / 403** at the boundary (set it with the editor's auth selector, or wire
  ...
  trigger can reach it without `requireAuth`, so a forgotten gate is caught while
  the op stays pure.
...
inside, *works*. That's exactly why it's the seductive wrong turn. It
...
edges in place of one opaque blob.
...
several ops. Don't use it to rebuild the entity the op already returned. Exploding an
