...
is a workflow: author it in the admin, drop it in `workflows/`, or build
it from code.
...
in config and the schedule re-arms. No separate job runner.
...
a **hook** (`core.hook.invoke` ↔ `boundary.hook`) when you need an
...
backpressured copies: an agent's token stream to both an SSE response
