...
Workflows are *data*. Ops carry the code. The engine runs a typed
...
1. **Workflows are data.** Op references and config; the engine never evals anything.
2. **Ops carry the code.** Each node is an instance of an op: typed input/output ports and an `execute`.
...
Edge kind is **derived from port kinds**: the ports you connect determine it.
