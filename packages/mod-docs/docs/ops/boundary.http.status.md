...
carry an `error` field (say, a run's failure message) stays a 200 response;
...
`status`/`body` into `boundary.http.response`. One node handles the mapping
per route (see *Designing your API*).
