...
— named control-outs, not data. The un-pulsed branch's whole region is marked
*skipped* and settles cleanly (skip propagates through value, stream, and
control edges; no hanging).
...
value port instead (a value input with several producers resolves to
