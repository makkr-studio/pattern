...
run** — extra spans, but **no sub-run and no new Runs entry** (the power of
`core.stream.map` without the noise). Outputs the current `item` and its `index`;
...
sub-run ops — that's the no-sub-run boundary). In Replay the region nodes pulse
once per chunk.
