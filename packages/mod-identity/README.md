...
mods that plug into it — `@pattern/mod-auth-magic-link` is the reference.

```bash
npm install @pattern/mod-identity
```
...
exactly as before. Installing it alongside `@pattern/mod-admin` **flips the admin
...
A login method mod — typically **`@pattern/mod-auth-magic-link`**. Identity owns
