/**
 * @pattern/mod-identity — service access for ops.
 *
...
import { IDENTITY_SERVICE, type OpContext } from "@pattern/core";
...
export function identityService(ctx: OpContext): IdentityService {
  ...
  if (!svc) throw new Error("identity service is not registered (install @pattern/mod-identity)");
  return svc;
}
