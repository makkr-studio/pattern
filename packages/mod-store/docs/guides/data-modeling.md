...
`locked: true` field (you'd race on it — that's exactly what a lease prevents).
...
because the engine simply won't run a filter on a field you didn't declare.
...
`ok:false` means someone wrote first, so re-read and retry rather than
...
branch on (wire it into `core.flow.branch`), never an exception.
...
"settles" before your work truly finishes — own the lease with an id *you*
control rather than the runId, then `store.lease.release` at your terminal
event. A streaming response settles when its stream is *captured*, not drained;
the chat pipeline therefore owns `chat:conversation:{id}` as `turn:{turnId}`
(not the runId), renews while events flow, and releases when the turn ends — so
...
served by the shipped `GET /store/blobs/:id`, or `store.blob.get` wired into
your own response). Store the returned id (and any metadata you'll filter on)
in a document; keep the bytes out of the document body. Blobs aren't
garbage-collected — when you `store.delete` a record that owns a blob, delete
the blob too (`store.blob.delete`) or the bytes leak.
