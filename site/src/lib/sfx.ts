/**
 * SFX — a tiny WebAudio synthesizer giving the site a voice, lifted from the
 * Pattern admin. No samples, no assets: every sound is a few oscillators with an
 * envelope, so the whole soundscape costs ~0 bytes and never blocks. Volumes are
 * deliberately low — texture, not noise.
 *
